...
extern "C"
{
    __attribute__((sdx_kernel("poseidon_hash", 0))) void poseidon_hash(
        const ap_uint<256> *input,
        ap_uint<256> *output,
        int num_blocks)
    {
#line 8 "/home/terryadavis/utkarsh/zkml-fpga/hls/hash_pe/ccs/run_hls.tcl"
#pragma HLSDIRECTIVE TOP name=poseidon_hash
# 18 "../src/poseidon.cpp"
...
#pragma HLS pipeline II = 1
 ...
 ap_uint<256> state[3];
            VITIS_LOOP_35_1: for (int i = 0; i < 3 - 1; ++i)
            {
                state[i] = input[blk * (3 - 1) + i];
            }
            state[3 - 1] = 0;

        round_loop:
            for (int r = 0; r < 8 + 57; ++r)
            {
                VITIS_LOOP_44_2: for (int i = 0; i < 3; ++i)
                {
#pragma HLS unroll
 ap_uint<256> tmp = state[i] + rc[r * 3 + i];
                    state[i] = (tmp >= p) ? (element)(tmp - p) : (element)tmp;
                }
                if (r < 8 / 2 || r >= 8 / 2 + 57)
                {
                    VITIS_LOOP_52_3: for (int i = 0; i < 3; ++i)
                    {
#pragma HLS unroll
 ap_uint<256> x2 = (state[i] * state[i]) % p;
                ...
                ap_uint<256> tmp_state[3];
                VITIS_LOOP_67_4: for (int i = 0; i < 3; ++i)
                {
#pragma HLS unroll
 ap_uint<256> acc = 0;
                    VITIS_LOOP_71_5: for (int j = 0; j < 3; ++j)
                    {
#pragma HLS unroll
 acc += state[j] * mds[i][j];
                    }
                ...
                VITIS_LOOP_78_6: for (int i = 0; i < 3; ++i)
                {
#pragma HLS unroll
 state[i] = tmp_state[i];
                }
